...
import com.xqxls.swagger.*;
import com.xqxls.util.JavaTypes;
import com.xqxls.util.ModelInfo;
import com.xqxls.util.StringUtils;
...
public class TemplateBuilder {
    ...
    private static String COLUMN_DESC_SQL = "SELECT column_name, comments FROM all_col_comments WHERE table_name = ? AND owner = ?";

    private static String TABLE_DESC_SQL = "SELECT comments FROM all_tab_comments WHERE table_name = ? AND owner = ?";
    ...
    public static void builder(){
        try {
            ...
            String inputTableNameStr = props.getProperty("tableName");
            if(Objects.isNull(inputTableNameStr)){
                throw new ApiException("请输入表名");
            }
            String[] tableList = inputTableNameStr.split(",");
            if(tableList.length == 0){
                throw new ApiException("处理后的表为空，请重新输入");
            }
            for(String inputTableName:tableList){
                //针对MySQL数据库进行相关生成操作
                if(databaseType.equals("MySQL")){
                    //获取所有表结构
                    ResultSet tableResultSet = metaData.getTables(null, "%", inputTableName, new String[]{"TABLE"});
                    ...
                    while (tableResultSet.next()){
                        ...
                        if (rs != null && rs.next()) {
                            ...
                            tableComment = parse(createDDL).replace("\"","\\\"").replace("/","-");
                        }
                        ...
                        while (cloumnsSet.next()){
                            ...
                            String nullAble = cloumnsSet.getString("IS_NULLABLE");
                            ...
                            //创建该列的信息
                            ModelInfo modelInfo = new ModelInfo(javaType, JavaTypes.simpleName(javaType), propertyName, StringUtils.firstUpper(propertyName), remarks, key.equals(columnName), columnName, cloumnsSet.getString("IS_AUTOINCREMENT"));
                            modelInfo.setNullAble(nullAble);
                        ...
                        modelMap.put("time",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
//
                        //创建JavaBean
                ...
                if(databaseType.equals("Oracle")){
                    ...
                    // 查询表描述
                    String tableQuery = TABLE_DESC_SQL;
                    PreparedStatement tableStatement = conn.prepareStatement(tableQuery);
                    tableStatement.setString(1, inputTableName.toUpperCase());
                    tableStatement.setString(2, UNAME.toUpperCase());
                    ResultSet resultTableDescSet = tableStatement.executeQuery();
                    String tableComment = "";
                    while (resultTableDescSet.next()) {
                        tableComment = resultTableDescSet.getString("COMMENTS");
                    }

                    // 查询所有列字段的描述
                    String query = COLUMN_DESC_SQL;
                    PreparedStatement statement = conn.prepareStatement(query);
                    statement.setString(1, inputTableName.toUpperCase());
                    statement.setString(2, UNAME.toUpperCase());
                    ResultSet resultColumnDescSet = statement.executeQuery();
                    Map<String,String> columnDescMap = new HashMap<>();
                    while (resultColumnDescSet.next()) {
                        String columnName = resultColumnDescSet.getString("COLUMN_NAME").toLowerCase();
                        String columnDesc = resultColumnDescSet.getString("COMMENTS");
                        columnDescMap.put(columnName,columnDesc);
                    }
                    //名字操作,去掉tab_,tb_，去掉_并转驼峰
                    String table = StringUtils.replace_(StringUtils.replaceTab(inputTableName));
                    //大写对象
                    String Table =StringUtils.firstUpper(table);
                    ...
                    //获取表所有的列
                    ResultSet cloumnsSet = metaData.getColumns(null, UNAME.toUpperCase(), inputTableName.toUpperCase(), null);
                    //获取主键
                    ResultSet keySet = metaData.getPrimaryKeys(null, UNAME.toUpperCase(), inputTableName.toUpperCase());
                    String key ="",keyType="java.lang.Long";
                    while (keySet.next()){
                        key=keySet.getString(4).toLowerCase();
                    }
                    ...
                    while (cloumnsSet.next()){
                        String nullAble = cloumnsSet.getString("IS_NULLABLE");
                        String columnName = cloumnsSet.getString("COLUMN_NAME").toLowerCase();
                        // 列的描述
                        String remarks = columnDescMap.get(columnName);
                        //处理列名
                        String propertyName = StringUtils.replace_(columnName);
                        //获取类型，并转成JavaType
                        int columnSize = cloumnsSet.getInt("COLUMN_SIZE");
                        int scale = cloumnsSet.getInt("DECIMAL_DIGITS");
                        String javaType = JavaTypes.getType(cloumnsSet.getInt("DATA_TYPE"),columnSize,scale);
                        //创建该列的信息
                        ModelInfo modelInfo = new ModelInfo(javaType, JavaTypes.simpleName(javaType), propertyName, StringUtils.firstUpper(propertyName), remarks, key.equals(columnName), columnName, "NO");
                        modelInfo.setNullAble(nullAble);
                        models.add(modelInfo);
                        //需要导包的类型
                        typeSet.add(javaType);
                    ...
                    modelMap.put("Table",Table);
                    modelMap.put("swagger",SWAGGER);
                    modelMap.put("TableName",inputTableName);
                    modelMap.put("models",models);
                    modelMap.put("typeSet",typeSet);
                    //主键操作
                    modelMap.put("keySetMethod","set"+StringUtils.firstUpper(StringUtils.replace_(key)));
